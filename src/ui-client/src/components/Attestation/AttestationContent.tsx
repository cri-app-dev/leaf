...
export default class AttestationContent extends React.PureComponent<Props> {
    ...
    public render() {
        ...
        const {
            handleApprovalTypeClick,
            handleUseTypeClick,
            handleDocumentationTitleChange,
            handleDocumentationExpDateChange,
            handleDocumentationInstitutionChange,
            handlePhiTypeClick,
            show,
            parentState,
        } = this.props;
        ...
        return (
            <div className={contentClass}>
                ...
                {((sessionType === SessionType.Research && hasApprovedIrb) ||
                    (sessionType === SessionType.QualityImprovement && hasApprovedQi)) && (
                    <AttestationRow className={`${c}-row doc-input`}>
                        <FormGroup>
                            <Label for={`${c}-doc-number`}>
                                {sessionType === SessionType.Research ? irbIdLabel : qiIdLabel}
                            </Label>
                            <Input
                        ...
                        <FormGroup>
                            ...
                            <Input
                                ...
                                placeholder={
                                    sessionType === SessionType.Research ? irbInstPlaceholder : qiInstPlaceholder
                                }
                                spellCheck={false}
                        ...
                        <FormGroup>
                            ...
                            {documentationExpDateString && !attestation.documentation.expirationDate && (
                                <span className={`${c}-invalid-date`}>
                                    Enter a future date in the format "MM/DD/YYYY"
                                </span>
                            )}
                        </FormGroup>
                    </AttestationRow>
    ...
    private setUseTypeButtonClass = (sessionType: SessionType) => {
        return this.props.parentState.sessionTypeSelected &&
            this.props.parentState.attestation.sessionType === sessionType
    ...
    private setDocumentationApprovedClass = (hasApproval: boolean) => {
        const approved =
            this.props.parentState.attestation.sessionType === SessionType.Research
